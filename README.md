...
새 버전 DMG를 설치하면 헬퍼가 자동으로 재설치됩니다 (관리자 비밀번호 1회).

> v1.5.1 이하에서 업그레이드 시 캘린더 권한을 재설정해야 할 수 있습니다. 캘린더 토글이 동작하지 않으면 터미널에서 `tccutil reset Calendar com.zerolive.BatteryAgent` 실행 후 앱을 재시작하세요.
